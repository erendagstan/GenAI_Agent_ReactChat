...
st.set_page_config(page_title="Chat Interaction with ReAct Agent")
st.title("Chat Interaction with ReAct Agent")
st.divider()

st.sidebar.header("Agent Configuration")
st.sidebar.divider()
selected_llm = st.sidebar.radio(label="Select Language Model", options=["GPT-4", "Gemini Pro", "Claude 2.1"])
st.sidebar.divider()
selected_search_engine = st.sidebar.radio(label="Select Search Engine", options=["DuckDuckGo", "Tavily"], index=1)
st.sidebar.divider()
selected_image_generator = st.sidebar.radio(label="Select Image Generation Model",
                                            options=["Stable Diffusion XL", "DALL-E 3"])
st.sidebar.divider()
selected_web_scraper = st.sidebar.radio(label="Select Web Scraping Tool", options=["BeautifulSoup"])
st.sidebar.divider()
turkish_sensitivity = st.sidebar.checkbox(label="Force Response in Turkish", value=True)
st.sidebar.divider()
...
if prompt := st.chat_input(placeholder="Enter your message"):
    ...
    with st.chat_message("assistant"):
        st.info("🧠 Processing Thought Chain...")
